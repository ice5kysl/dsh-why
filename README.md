...
- **Graph-row aware, three-tier verdicts** — the loader resolves `require()` via seed words → materialized modules → **registered factories**: every mounted `dsh.client` package registers a factory under its package name when its combo batch executes. dsh-why scans your local dsh install for those rows and classifies each require as *resolvable* (seed / immediate row / declared lazy row), *conditional* (an undeclared lazy row — usually resolves by batch timing; declaring it in `dsh.client.external` makes it deterministic), or *missing* (never shipped anywhere — the crash class). Conditional is a warning, never a red card. Comments/strings and bundle-local relative requires are never misread as missing modules.
