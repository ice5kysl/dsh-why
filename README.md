...
4. **Degrades gracefully**: `--offline` (or an unreachable network) falls back to the bundled shell-history snapshot and the local rule base; an unreadable install tree declassifies its own verdicts to *unclassified* warnings instead of inventing crashes. The env block always states which row model ran (`shell graph rows: 9 immediate + 37 lazy` / `scan-only` / `UNREADABLE`) — the model is part of the answer. A diagnostic tool must never itself crash, and it must never be more confident than its inputs.
