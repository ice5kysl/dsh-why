 ...
 *
 * `seeded` marks a signature that so far has **only** cold-start seed rows
 * (machine-measured, scripts/seed-corpus.mjs) and no user reports — the
 * renderer must not present that as "N report(s)".
 *
 * A crash-level finding whose sig is NOT in the corpus gets `corpusMiss` —
 * the best moment to ask for a --share report. Only set when the corpus was
 * actually loaded: offline or unreachable means "unknown", never "miss".
 */
