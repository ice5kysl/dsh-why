 ...
 * Scanned from the LOCAL dsh install in two steps, so a partial tree still
 * answers:
 *   1. every package in the install that declares a dsh.client face (its own
 *      `immediately` flag travels with it);
 *   2. intersected with the composed roster parsed from the in-box bundles'
 *      cordis.patch.yml files — that filter drops client-capable packages this
 *      shell does NOT mount (e.g. the alternative directory pickers).
 *
 * `exact: true` means step 2 applied (the mounted set). When the roster files
 * are unreadable but the scan worked, the scan result is returned with
 * `exact: false`: a few unmounted alternates may then be called conditional
 * instead of missing — the harmless direction. `null` (nothing readable at
 * all) is the caller's signal that the row branch is unobservable, which makes
 * classification answer 'unknown' instead of a false crash report.
 */
export function collectShellRows(npmRoot) {
  ...
  try {
    ...
    if (!clientPkgs.size) return null // nothing readable — the row branch is unobservable
    const exact = roster.size > 0
    const immediate = []
    const lazy = []
    for (const [name, meta] of [...clientPkgs.entries()].sort()) {
      if (exact && !roster.has(name)) continue // client face but not mounted → not a graph row
      ;(meta.immediately ? immediate : lazy).push(name)
    }
    return { immediate, lazy, exact }
  } catch {
    return null
  }
}
...
/**
 * Read ONE plugin package directory — the author's checkout, not an installed
 * profile. Returns the same shape as readProfile (one enabled plugin) so it can
 * flow through the same diagnose() rule base. This is the `--package` pre-publish
 * self-check: client bundle + engines.dsh + module-table classification, no
 * install required beyond a dsh to check against.
 */
export function readPackage(dir) {
  const abs = resolve(dir)
  const fallback = abs.split(/[\\/]/).filter(Boolean).pop() || 'package'
  const empty = { profile: fallback, dir: abs, manifestFound: false, baseline: 0, plugins: [], leftovers: [] }
  const manifest = readJson(join(abs, 'package.json'))
  if (manifest === null) return empty

  const name = typeof manifest.name === 'string' && manifest.name ? manifest.name : fallback
  const version = typeof manifest.version === 'string' && manifest.version ? manifest.version : null
  const enginesDsh = typeof manifest.engines?.dsh === 'string' ? manifest.engines.dsh : null
  ...
  let client = null
  // A declared client whose bundle cannot be found or read (the usual cause: the
  // checkout has not been built yet) must stay distinguishable from "this plugin
  // has no client bundle at all". Collapsing the two made `--package` answer
  // "all clear" on a repo where it had checked nothing.
  const clientDeclared = Boolean(manifest.dsh?.client)
  let clientEntry = null
  const entry = clientEntryPath(manifest) ?? (existsSync(join(abs, 'lib', 'client.js')) ? 'lib/client.js' : null)
  if (entry) {
    clientEntry = entry
    const bundlePath = join(abs, entry)
    try {
      const text = readFileSync(bundlePath, 'utf8')
      const { requires, requiresV2, local } = extractRequiresV2(text)
      client = { entry, requires, requiresV2, local }
    } catch { /* declared but unreadable — reported as unverifiable, never as "no client" */ }
  }
  ...
  return {
    ...
    plugins: [{
      name,
      spec: `file:${abs}`, // a local dir has no npm "latest" — keeps R3 quiet
      version,
      enginesDsh,
      plugin: true,
      materialized: true,
