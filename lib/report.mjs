...
function renderFinding(s, c, finding) {
  ...
  if (finding.rule === 'R1' && finding.conditional) {
    lines.push(c.yellow(head + s.r1CondTitle(finding.plugin)))
    lines.push(`  ${s.r1CondDetail(finding.conditional)}`)
    lines.push(`  ${s.r1CondFix}`)
    return lines
  }
  if (finding.rule === 'R1') {
    lines.push(c.red(head + (finding.plugin ? s.r1Title(finding.plugin) : s.r1TitleError)))
