...
// UTM-tagged entry points, so Umami can attribute where a visit came from.
// Only links a human clicks carry params — data endpoints and User-Agent
// strings are fetched, not clicked, and never get them.
const SITE = {
  issue: 'https://dsh-why.com?utm_source=dsh-why&utm_medium=issue',
  prompt: 'https://dsh-why.com?utm_source=dsh-why&utm_medium=prompt',
}
const topicUrl = (topic) => `https://dsh-why.com${topic}?utm_source=dsh-why&utm_medium=cli`
...
function renderFinding(s, c, finding, fixes = null, lang = 'en') {
  ...
  const lines = []
  if (finding.rule === 'R7') {
    const title = finding.reason === 'package-not-found' ? s.r7NotFoundTitle(finding.plugin)
      : finding.reason === 'client-bundle-missing' ? s.r7MissingTitle(finding.plugin)
        : s.r7UnbuiltTitle(finding.plugin)
    const detail = finding.reason === 'package-not-found' ? s.r7NotFoundDetail
      : finding.reason === 'client-bundle-missing' ? s.r7MissingDetail
        : s.r7UnbuiltDetail(finding.entry)
    lines.push(c.yellow(head + title))
    lines.push(`  ${detail}`)
    lines.push(`  ${c.dim(s.r7WhyItMatters)}`)
    lines.push(`  ${s.r7Fix}`)
    return lines
  }
...
export function buildIssueTemplate(report, lang, toolVersion) {
  ...
  const blocks = broken.map((f) => {
    ...
    const lines = [
      s.issueHeading,
      '',
      s.issueTool(toolVersion, SITE.issue),
...
export function buildFixPrompt(report, lang, toolVersion) {
  ...
  lines.push(s.issueTool(toolVersion, SITE.prompt), '', s.promptFindingsLabel)
...
export function renderText(report, lang, toolVersion, { color = false, showPrompt = false, rowsNote = null, hint = true } = {}) {
  ...
  if (report.findings.length) {
    ...
    for (const finding of report.findings) {
      ...
      // Community crash corpus: "others hit this exact crash" — right under the
      // finding it belongs to; a miss (corpus loaded, sig absent) gets the
      // --share nudge instead; offline/unreachable corpus → no line at all.
      // A signature that has only cold-start seed rows (seeded, count 0) is a
      // known crash *pattern*, not an "N report(s)" claim — say so honestly.
      if (finding.corpusHit) {
        const hit = finding.corpusHit
        if (hit.count > 0) {
          const last = String(hit.lastSeen ?? '').slice(0, 10) || '—'
          lines.push(c.dim(`  ${s.corpusHit(hit.count, last)}`))
        } else if (hit.seeded) {
          lines.push(c.dim(`  ${s.corpusSeeded}`))
        }
      } else if (finding.corpusMiss) {
        // A miss is the best moment to ask for a report — only set when the
        // corpus was really loaded (offline/unreachable leaves it unknown).
        lines.push(c.dim(`  ${s.corpusMiss}`))
      }
    }
    lines.push('')
  }
  ...
  if (report.errorRefs) {
    ...
    if (report.summary.unclassified) {
      // Two very different causes share the count: unreadable shell graph rows
      // (the environment's fault) and a --package gate that could not read what
      // it was asked to check (the author's). Only the second changes an exit code.
      const gateBlocked = report.findings.some((f) => f.rule === 'R7')
      extra.push(gateBlocked ? s.summaryUnverifiable(report.summary.unclassified) : s.summaryUnclassified(report.summary.unclassified))
    }
...
function renderRunFinding(s, c, run, { evidence = null, showTurn = false } = {}) {
  ...
  if (run.topic) lines.push(`  ${c.dim(s.runNextStepTopic(topicUrl(run.topic)))}`)
  return lines
}
