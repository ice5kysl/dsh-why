...
/**
 * The known-fix case-base block: status + recipe + real-world cases per module.
 * The title says so when the recipes came from the bundled snapshot rather than
 * the live site, so a reader never mistakes a stale recipe for current guidance.
 */
function renderKnownFixes(s, finding, fixes) {
 ...
 */
function renderFinding(s, c, finding, fixes = null) {
  ...
  if (finding.rule === 'R1' && finding.reason) {
    ...
    if (conditional.length) {
      ...
      lines.push(...renderKnownFixes(s, finding, fixes))
    }
    if (unknown.length) {
...
export function renderText(report, lang, toolVersion, { color = false, showPrompt = false } = {}) {
  ...
  if (report.findings.length) {
    ...
    for (const finding of report.findings) {
      lines.push(...renderFinding(s, c, finding, report.fixes))
    }
    lines.push('')
  }
