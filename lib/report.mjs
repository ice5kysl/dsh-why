...
function renderFinding(s, c, finding, fixes = null, lang = 'en') {
  ...
  if (finding.rule === 'R1') {
    ...
    let title
    if (!finding.fromError) {
      title = finding.plugin ? s.r1Title(finding.plugin) : s.r1TitleError // profile mode
    } else if (finding.plugin && state === false) {
      title = s.r1Title(finding.plugin) // a pasted plugin AND a module this shell truly lacks → that plugin crashes
    } else {
      title = state === true ? s.r1TitleErrorResolved
    ...
    if (state === 'unknown') {
      ...
      lines.push(`    1. ${s.r1UnknownFix}`)
    } else if (state === false || state === undefined) {
      lines.push(`  ${s.r1FixTitle}`)
      let i = 0
      const add = (text) => lines.push(`    ${++i}. ${text}`)
      if (finding.upgradeAvailable && finding.latest) add(s.r1FixUpgradePlugin(finding.plugin, finding.latest))
      ...
      if (addedLater) add(s.r1FixUpgradeDsh(finding.history[addedLater].since))
      if (removed) add(s.r1FixDowngradeDsh(finding.history[removed].last))
      if (finding.plugin) add(s.r1FixRemove(finding.plugin))
      add(s.r1FixIssue)
    }
