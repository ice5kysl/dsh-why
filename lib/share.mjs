...
/**
 * The deterministic crash signature of one finding — the same value the
 * collector aggregates by, so a local diagnosis can be looked up in the
 * community crash corpus (and a --share report lands on the same entry).
 */
export function findingSig(f) {
  ...
  return `${String(f.rule ?? 'rx').toLowerCase()}_${hash}`
}
