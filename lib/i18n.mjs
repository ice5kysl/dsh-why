...
const dict = {
  en: {
    ...
    envPackage: (name, version, hasClient) => `package: ${name}${version ? `@${version}` : ''} — pre-publish self-check${hasClient ? '' : ' (no client bundle found)'}`,
    envPackageMissing: (name) => `package: ${name} — not found (no package.json at that path)`,
    ...
    corpusHit: (count, last) => `Community corpus: ${count} report(s) of this exact crash (latest ${last}) · --share adds yours`,
    corpusSeeded: 'Known crash pattern: measured in the upstream corpus, no user reports yet · --share adds yours',
    corpusMiss: 'Not in the community corpus yet · --share it and help the next person who hits this',
    ...
    summaryUnverifiable: (n) => `${n} check(s) could NOT be performed — see R7 (a pass was not earned, so this is not exit 0)`,
    ...
    errUnknownAdvice: 'Generic steps: 1) run npx dsh-why for a full diagnosis of this machine; 2) look the plugin up on https://dsh-insights.com?utm_source=dsh-why&utm_medium=cli; 3) try upgrading or downgrading dsh.',
    ...
    issueTool: (v, url) => `- diagnosed by: dsh-why v${v} (${url})`,
    ...
    // ---- `dsh-why run`: attribute a finished (or aborted) run ----------------
    // ---- R7: the --package gate could not perform its check -------------------
    r7NotFoundTitle: (dir) => `no package.json at ${dir} — the gate checked nothing`,
    r7NotFoundDetail: 'Point --package at a directory that actually contains the plugin\'s package.json.',
    r7UnbuiltTitle: (name) => `${name}: dsh.client is declared, but the bundle could not be read — the gate checked nothing`,
    r7UnbuiltDetail: (entry) => `dsh.client resolves to ${entry}, which is missing or unreadable. The usual cause is an unbuilt checkout: the client bundle is a build artifact and is not committed.`,
    r7MissingTitle: (name) => `${name}: dsh.client is declared, but no client bundle could be located — the gate checked nothing`,
    r7MissingDetail: 'dsh.client is set, yet neither exports["./client"] nor the conventional lib/client.js resolves to a file. Build the plugin, or point exports["./client"] at the bundle.',
    r7WhyItMatters: 'A pre-publish gate that cannot read the bundle has verified nothing. Reporting that as "all clear" is the one answer it must never give.',
    r7Fix: 'Build first, then re-run: npm run build && npx dsh-why --package .',
    runTitle: (v) => `dsh-why v${v} — dsh (DeepSeek Harness) run diagnostics`,
    runScopeLatest: 'scope: your latest session',
    runScopeAll: (n) => `scope: all sessions (newest ${n} log(s))`,
    runScan: (sessions, turns, mb, ms) => `scanned: ${sessions} session(s) · ${turns} turn(s) · ${mb} MB · ${ms} ms`,
    runScanFrames: (decoded, skipped) => `logs: ${decoded} zstd frame(s) decoded${skipped ? ` (${skipped} further magic-byte hit(s) were payload coincidences, not frame starts)` : ''} — this format is a frame chain, not one frame; a single-frame read would silently drop the rest`,
    runScanSuperseded: (n) => `${n} superseded log(s) left behind by a format migration were elected away, not counted twice`,
    runScanMidFlight: (n) => `${n} log(s) stopped mid-run (last event is not turn/end) — that is a crash, not a read error`,
    runScanUnbalanced: (n) => `${n} log(s) have unpaired step/tool events; read as-is and flagged, never silently repaired`,
    runScanNoise: (chunk, lines, pct) => `${pct}% of scanned lines (${chunk} of ${lines}) were unsequenced stream chunks — counted, then dropped as noise`,
    runLatestTitle: 'Latest run',
    runLatestLine: (turn, iso, calls, results, steps) => `turn ${turn}${iso ? ` · ${iso}` : ''} · ${calls} tool call(s), ${results} result(s), ${steps} step(s)`,
    runModel: (m) => `model: ${m}`,
    runVerdictLine: (verdict, cls, rule, confidence) => `verdict: ${verdict} · ${cls}${rule ? ` (${rule})` : ''} · confidence: ${confidence}`,
    runProviderSaid: (msg) => `provider said: ${msg}`,
    runLogLine: (format, full, complete, midFlight, lastEvent) => `log: ${format} format · frames ${full ? 'fully' : 'PARTIALLY'} decoded · ${complete ? 'consistent' : 'UNPAIRED events present'}${midFlight ? ` · stopped mid-run (last: ${lastEvent})` : ''}`,
    runHealthy: (n) => `No failed run in scope — ${n} turn(s) checked.`,
    runNoLogs: 'No dsh session logs found under DSH_HOME — nothing to diagnose yet.',
    runNoLogsHint: 'Session logs live at <DSH_HOME>/sessions/<workspace>/<session-id>/session.jsonl.zstd. Run dsh once, then retry.',

    runFindingsTitle: 'Findings',
    runAbortTitle: 'this turn was stopped deliberately — not a failure',
    runAbortDetail: (by) => `turn/end.reason = {"kind":"aborted","reason":{"kind":"${by ?? 'user'}"}} — dsh-why does not raise a red card for a stop you asked for.`,
    runUnclassifiedTitle: {
      interrupted: 'this turn ended without a verdict',
      'orphan-turn': 'this run never finished',
      'unknown-kind': 'the log recorded a turn end dsh-why does not recognise',
    },
    runUnclassifiedDetail: {
      interrupted: 'turn/end.reason = {"kind":"interrupted"} — the log does not say whether the process was killed, the window closed, or the run crashed.',
      'orphan-turn': 'the turn has a turn/start but no turn/end, so nothing recorded how it ended.',
      'unknown-kind': 'the turn/end.reason shape is not in the catalogued vocabulary.',
    },
    runUnclassifiedPromise: 'I will not guess: an unfinished turn is not evidence of a broken plugin.',
    runEvidenceMix: (n, total, pct) => `evidence: ${n} of ${total} failure(s) in scope are this same class (${pct}%)`,
    runEvidenceChronic: (days, sessions) => `this class appears on ${days} day(s) across ${sessions} session(s) — a chronic background condition, not a one-off`,
    runEvidenceFirst: 'this is the first appearance of this class in the scanned window',
    runSideEffectNone: (planned) => `the turn died before any of its ${planned} tool call(s) ran — nothing was written to disk`,
    runSideEffectPartial: (executed, planned) => `${executed} of ${planned} tool call(s) had already run before it died — files may be half-written`,
    runRecoveryTitle: 'Recovery:',

    runClass: {
      ok: 'completed',
      'user-abort': 'stopped by you',
      interrupted: 'interrupted — the log holds no verdict',
      'orphan-turn': 'never finished (no turn/end)',
      'unknown-kind': 'unrecognised turn end',
      'provider-transport': 'the API call never completed',
      'provider-server': 'the provider returned a server error',
      'provider-auth': 'the provider rejected your credentials',
      'provider-missing-credential': 'no API key is configured for this route',
      'provider-quota': 'the account is out of credit',
      'provider-pricing': 'this model has no pricing configured',
      'model-unavailable': 'the provider does not serve this model',
      'provider-timeout': 'the response stream went idle and timed out',
      'provider-unknown': 'the provider errored in a way dsh-why has not catalogued',
    },
    runFixes: {
      'provider-transport': [
        're-run the turn — this class is the network or the provider endpoint, not a plugin',
        'if it repeats, check the proxy/VPN path to the API host and try again',
        'confirm with `npx dsh-why` that the local install itself is healthy',
      ],
      'provider-server': [
        're-run the turn — a 5xx from the provider is upstream, nothing local to fix',
        'if it persists, switch model from the composer and retry',
      ],
      'provider-auth': [
        'replace the key: the provider says it is invalid or expired',
        '`npx dsh-why` reports the key route and where dsh reads it from',
      ],
      'provider-missing-credential': [
        'set the key for this route (the web Models page writes it, or export it in the environment dsh boots from)',
        'the message names the exact variable to store — set that one',
      ],
      'provider-quota': ['top up the account — the provider rejected the request for insufficient balance'],
      'provider-pricing': [
        'pick a model that has pricing configured for this key, or have the route configured',
        'this usually appears right after a model switch — the old key may not cover the new model',
      ],
      'model-unavailable': [
        'choose a model this key can serve — `400 unsupported model` means the route does not offer it',
        'if you just switched models, switch back or update the route first',
      ],
      'provider-timeout': [
        're-run the turn — an idle stream timeout is usually transient',
        'for long tool-heavy turns, raise the timeout rather than retrying blindly',
      ],
      'provider-unknown': [
        're-run the turn once; if it repeats, the message below is the thing to report',
        'run with --json and attach the finding to https://github.com/ice5kysl/dsh-why/issues so the class can be catalogued',
      ],
    },

    runMixTitle: (n) => `Failure mix (${n} failure(s)):`,
    runMixRow: (label, n, pct) => `${label} — ${n} (${pct}%)`,
    runHealthLine: (completed, failed, aborted, unclassified) => `completed ${completed} · failed ${failed} · stopped by you ${aborted} · unclassified ${unclassified}`,
    runBaseline: (pct) => `your baseline: ${pct}% of turns fail on a normal day`,
    runIncidentsTitle: 'Incidents (days that differ from YOUR baseline):',
    runIncidentRate: (errors, turns, pct, multiple) => `${errors}/${turns} turns failed (${pct}%, ${multiple}× your baseline)`,
    runIncidentNovel: (classes) => `failure classes never seen on any other day: ${classes}`,
    runNoIncidents: 'no day stands out from your baseline — the failures above are chronic, not an event',
    runUnclassifiedSection: (n) => `Unclassified (${n}) — reported as unknown, never as failure:`,
    runAbortsLine: (n) => `${n} turn(s) were stopped by you — a deliberate stop is never a failure`,

    runNextStepsTitle: 'Next:',
    runNextStepTopic: (url) => `background and other fixes for this class: ${url}`,
    runNextStepAll: 'run `npx dsh-why run --all` for the health view — it separates chronic failures from incidents',
    runNextStepPaste: 'cannot run it where dsh lives? paste the error at https://dsh-why.com?utm_source=dsh-why&utm_medium=cli and get the same report',

    hintJson: 'Tip: --json for machine-readable output, --offline to skip all network, --lang zh for 中文. Docs: https://dsh-why.com?utm_source=dsh-why&utm_medium=cli',
  },
  ...
  zh: {
    ...
    envPackage: (name, version, hasClient) => `包：${name}${version ? `@${version}` : ''} —— 发版前自检${hasClient ? '' : '（未找到 client bundle）'}`,
    envPackageMissing: (name) => `包：${name} —— 未找到（该路径没有 package.json）`,
    envProfileMissing: (name, available) => available.length
    ...
    corpusHit: (count, last) => `社区语料：已见 ${count} 例上报（最近 ${last}）· --share 可上报你的案例`,
    corpusSeeded: '已知崩溃模式：上游实测语料命中，暂无用户上报 · --share 可补充你的案例',
    corpusMiss: '社区语料还没有这个崩溃的记录 · --share 上报它，帮下一个遇到的人',
    r1FixTitle: '修法（任选其一）：',
    ...
    summaryUnverifiable: (n) => `有 ${n} 项检查无法执行 —— 见 R7（没挣到的通过不算通过，所以退出码不是 0）`,
    ...
    errUnknownAdvice: '通用排查：1) 跑 npx dsh-why 全量诊断本机；2) 上 https://dsh-insights.com?utm_source=dsh-why&utm_medium=cli 查该插件详情页；3) 试试升/降 dsh 版本。',
    ...
    issueEnv: (dsh, shell, profile) => `- dsh：${dsh} · shell：${shell} · profile：${profile}`,
    issueTool: (v, url) => `- 诊断工具：dsh-why v${v}（${url}）`,
    ...
    // ---- `dsh-why run`：给一次已结束（或被中断）的 run 定性 --------------------
    // ---- R7：--package 门禁没能执行检查 -------------------------------------
    r7NotFoundTitle: (dir) => `${dir} 下没有 package.json —— 门禁什么都没检查`,
    r7NotFoundDetail: '把 --package 指向真正包含该插件 package.json 的目录。',
    r7UnbuiltTitle: (name) => `${name}：声明了 dsh.client，但 bundle 读取失败 —— 门禁什么都没检查`,
    r7UnbuiltDetail: (entry) => `dsh.client 指向 ${entry}，该文件不存在或不可读。最常见的原因是仓库还没构建：client bundle 是构建产物，不会提交进仓库。`,
    r7MissingTitle: (name) => `${name}：声明了 dsh.client，但找不到任何 client bundle —— 门禁什么都没检查`,
    r7MissingDetail: '设了 dsh.client，但 exports["./client"] 和约定的 lib/client.js 都解析不到文件。请先构建插件，或把 exports["./client"] 指向真正的 bundle。',
    r7WhyItMatters: '一个读不到 bundle 的发版门禁，等于什么都没验证。把这种情形报成「全部通过」，是它最不该给出的答案。',
    r7Fix: '先构建再重跑：npm run build && npx dsh-why --package .',
    runTitle: (v) => `dsh-why v${v} —— dsh（DeepSeek Harness）运行诊断`,
    runScopeLatest: '范围：你最近的一次会话',
    runScopeAll: (n) => `范围：全部会话（最近 ${n} 个日志）`,
    runScan: (sessions, turns, mb, ms) => `已扫描：${sessions} 个会话 · ${turns} 个 turn · ${mb} MB · ${ms} ms`,
    runScanFrames: (decoded, skipped) => `日志：解出 ${decoded} 个 zstd 帧${skipped ? `（另有 ${skipped} 处魔数命中是压缩载荷里的巧合，不是帧起点）` : ''}——这个格式是帧链而非单帧，只读第一帧会静默丢掉其余全部内容`,
    runScanSuperseded: (n) => `${n} 个由格式迁移遗留下来的旧日志已被择优排除，没有重复计数`,
    runScanMidFlight: (n) => `${n} 个日志在 run 中途停止（最后一条事件不是 turn/end）——这是真崩溃，不是读取错误`,
    runScanUnbalanced: (n) => `${n} 个日志存在未配对的 step/tool 事件；照原样读取并标注，绝不静默修补`,
    runScanNoise: (chunk, lines, pct) => `${pct}% 的扫描行（${lines} 行中的 ${chunk} 行）是无 seq 的流式分片——只计数，当作噪声丢弃`,
    runLatestTitle: '最近一次 run',
    runLatestLine: (turn, iso, calls, results, steps) => `turn ${turn}${iso ? ` · ${iso}` : ''} · ${calls} 次工具调用，${results} 个结果，${steps} 个 step`,
    runModel: (m) => `模型：${m}`,
    runVerdictLine: (verdict, cls, rule, confidence) => `判定：${verdict} · ${cls}${rule ? `（${rule}）` : ''} · 置信度：${confidence}`,
    runProviderSaid: (msg) => `provider 原话：${msg}`,
    runLogLine: (format, full, complete, midFlight, lastEvent) => `日志：${format} 格式 · 帧${full ? '已全部' : '仅部分'}解出 · ${complete ? '事件配对一致' : '存在未配对事件'}${midFlight ? ` · run 中途停止（最后一条：${lastEvent}）` : ''}`,
    runHealthy: (n) => `范围内没有失败的 run —— 共检查 ${n} 个 turn。`,
    runNoLogs: 'DSH_HOME 下没有找到会话日志——还没有可诊断的东西。',
    runNoLogsHint: '会话日志位于 <DSH_HOME>/sessions/<工作区>/<会话 id>/session.jsonl.zstd。先跑一次 dsh 再试。',

    runFindingsTitle: '诊断结论',
    runAbortTitle: '这个 turn 是你主动停止的 —— 不算故障',
    runAbortDetail: (by) => `turn/end.reason = {"kind":"aborted","reason":{"kind":"${by ?? 'user'}"}} —— 你自己按的停，dsh-why 不会给你红牌。`,
    runUnclassifiedTitle: {
      interrupted: '这个 turn 结束了，但日志里没有给出结论',
      'orphan-turn': '这次 run 从未结束',
      'unknown-kind': '日志里的 turn 结束形态 dsh-why 不认识',
    },
    runUnclassifiedDetail: {
      interrupted: 'turn/end.reason = {"kind":"interrupted"} —— 日志没说清是进程被杀、窗口关闭，还是真的崩了。',
      'orphan-turn': '这个 turn 有 turn/start 却没有 turn/end，没有任何记录说明它是怎么结束的。',
      'unknown-kind': 'turn/end.reason 的形态不在已收录的词汇表里。',
    },
    runUnclassifiedPromise: '我不会猜：一个没跑完的 turn，不能当作插件坏了的证据。',
    runEvidenceMix: (n, total, pct) => `证据：范围内 ${total} 次失败中有 ${n} 次是同一类（${pct}%）`,
    runEvidenceChronic: (days, sessions) => `这一类分布在 ${days} 天、${sessions} 个会话 —— 是长期背景问题，不是偶发`,
    runEvidenceFirst: '这是本扫描窗口内该类别的首次出现',
    runSideEffectNone: (planned) => `turn 在 ${planned} 次工具调用中的任何一次执行前就死了 —— 没有写入任何东西`,
    runSideEffectPartial: (executed, planned) => `${planned} 次工具调用中已有 ${executed} 次执行完 —— 文件可能只写了一半`,
    runRecoveryTitle: '恢复办法：',

    runClass: {
      ok: '正常完成',
      'user-abort': '你主动停止',
      interrupted: '被中断 —— 日志里没有结论',
      'orphan-turn': '从未结束（没有 turn/end）',
      'unknown-kind': '无法识别的结束形态',
      'provider-transport': 'API 请求根本没完成',
      'provider-server': 'provider 返回了服务端错误',
      'provider-auth': 'provider 拒绝了你的凭证',
      'provider-missing-credential': '这条路由没有配置 API key',
      'provider-quota': '账户余额不足',
      'provider-pricing': '这个模型没有配置计价',
      'model-unavailable': 'provider 不提供这个模型',
      'provider-timeout': '响应流空闲超时',
      'provider-unknown': 'provider 报的错 dsh-why 尚未收录',
    },
    runFixes: {
      'provider-transport': [
        '重跑这个 turn —— 这一类是网络或 provider 端点的问题，跟插件无关',
        '如果反复出现，检查到 API 主机的代理/VPN 链路后重试',
        '用 `npx dsh-why` 确认本地安装本身是健康的',
      ],
      'provider-server': [
        '重跑这个 turn —— provider 的 5xx 在上游，本地没有可修的东西',
        '如果持续，在 composer 里换个模型再试',
      ],
      'provider-auth': [
        '换 key：provider 说它无效或已过期',
        '`npx dsh-why` 会告诉你 key 的路由以及 dsh 从哪读取它',
      ],
      'provider-missing-credential': [
        '给这条路由配上 key（web 的 Models 页面会写入，或在 dsh 启动的环境变量里导出）',
        '报错信息里点名了要存哪个变量 —— 就设那个',
      ],
      'provider-quota': ['给账户充值 —— provider 以「余额不足」拒绝了请求'],
      'provider-pricing': [
        '换一个这条 key 有计价配置的模型，或先把路由配置好',
        '这通常紧跟在切模型之后 —— 旧 key 可能不覆盖新模型',
      ],
      'model-unavailable': [
      ...
      'provider-timeout': [
        '重跑这个 turn —— 流空闲超时通常是暂时的',
        '对于工具很多的长 turn，应该调大超时，而不是盲目重试',
      ],
      'provider-unknown': [
        '先重跑一次；如果复现，下面这条报错就是该上报的东西',
        '用 --json 跑一次并把 finding 贴到 https://github.com/ice5kysl/dsh-why/issues，这个类别就能被收录',
      ],
    },

    runMixTitle: (n) => `失败构成（共 ${n} 次失败）：`,
    runMixRow: (label, n, pct) => `${label} —— ${n} 次（${pct}%）`,
    runHealthLine: (completed, failed, aborted, unclassified) => `完成 ${completed} · 失败 ${failed} · 你主动停止 ${aborted} · 无法定性 ${unclassified}`,
    runBaseline: (pct) => `你的基线：平常日子有 ${pct}% 的 turn 会失败`,
    runIncidentsTitle: '异常日（与「你自己的基线」显著不同的日子）：',
    runIncidentRate: (errors, turns, pct, multiple) => `${turns} 个 turn 里失败 ${errors} 个（${pct}%，是基线的 ${multiple} 倍）`,
    runIncidentNovel: (classes) => `此前任何一天都没出现过的失败类别：${classes}`,
    runNoIncidents: '没有任何一天显著偏离你的基线 —— 上面的失败是长期噪声，不是一次事件',
    runUnclassifiedSection: (n) => `无法定性（${n}）—— 明确报告为未知，绝不当作故障：`,
    runAbortsLine: (n) => `有 ${n} 个 turn 是你主动停止的 —— 主动停止永远不算故障`,

    runNextStepsTitle: '下一步：',
    runNextStepTopic: (url) => `这一类别的背景与其他修法：${url}`,
    runNextStepAll: '运行 `npx dsh-why run --all` 看健康视图 —— 它能把长期噪声和事件区分开',
    runNextStepPaste: '在 dsh 所在的机器上跑不了？把报错贴到 https://dsh-why.com?utm_source=dsh-why&utm_medium=cli，得到同一份报告',

    hintJson: '提示：--json 输出机器可读结果，--offline 完全不联网，--lang en 切换英文。文档：https://dsh-why.com?utm_source=dsh-why&utm_medium=cli',
  },
}
