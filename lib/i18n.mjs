...
const dict = {
  en: {
    ...
    r1KnownFixTitleBundled: 'Known fix (from the BUNDLED case-base snapshot — re-run online for the latest):',
    ...
    r1TitleErrorResolved: 'the module in the pasted error already resolves on this shell',
    r1TitleErrorConditional: 'the module in the pasted error is a built-in graph row of this shell',
    ...
    issueTool: (v) => `- diagnosed by: dsh-why v${v} (https://dsh-why.com)`,
    ...
    promptFixesLabelBundled: '## Known fixes (bundled case-base snapshot — offline, may lag the live site)',
    ...
    hintJson: 'Tip: --json for machine-readable output, --offline to skip all network, --lang zh for 中文. Docs: https://dsh-why.com',
  },
  ...
  zh: {
    ...
    r1KnownFixTitleBundled: '已知修法（来自包内案例库快照——联网重跑可拿最新）：',
    ...
    r1TitleErrorResolved: '报错中的模块在当前 shell 上已可解析',
    r1TitleErrorConditional: '报错中的模块是当前 shell 的内置图行',
    ...
    issueEnv: (dsh, shell, profile) => `- dsh：${dsh} · shell：${shell} · profile：${profile}`,
    issueTool: (v) => `- 诊断工具：dsh-why v${v}（https://dsh-why.com）`,
    ...
    promptFixesLabelBundled: '## 已知修法（包内案例库快照——离线，可能落后于线上）',
    ...
    hintJson: '提示：--json 输出机器可读结果，--offline 完全不联网，--lang en 切换英文。文档：https://dsh-why.com',
  },
}
