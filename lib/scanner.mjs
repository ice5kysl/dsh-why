 ...
 *   rows     { immediate: Set, lazy: Set } — the shell's client graph rows
 ...
 */
export function classify(spec, seed, pkgName, knownPkgs, rows = EMPTY_ROWS, declared = EMPTY_SET) {
...
export function resolvable(spec, seed, pkgName, knownPkgs, rows = EMPTY_ROWS, declared = EMPTY_SET) {
 ...
 *   (empty catch included) → ok, catch conditional → conditional, catch
 *   missing → broken; try conditional → catch fully ok → ok, else conditional.
 * Returns { status: 'ok' } | { status: 'conditional', conditional } |
 * { status: 'broken', missing } (ok versions record no lists: a guarded
 * require is not a defect).
 */
export function statusFor(requiresV2, seed, pkgName, knownPkgs, rows = EMPTY_ROWS, declared = EMPTY_SET) {
  const hardMissing = []
  const hardCond = []
  ...
  for (const e of pairRank.values()) {
    ...
    } else { // try === 1: likely resolves; if it throws, a catch with non-ok content leaves the pair conditional
