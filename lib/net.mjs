 ...
 *   - https://dsh-insights.com/data/crash-corpus.json     (community --share reports)
...
export const URLS = {
  ...
  shellRows: 'https://dsh-insights.com/data/shell-rows.json',
  compatObserved: 'https://dsh-insights.com/data/compat-observed.json',
  crashCorpus: 'https://dsh-insights.com/data/crash-corpus.json',
...
/**
 * The community crash corpus (opt-in --share reports, aggregated by
 * signature): advisory context for the diagnosis output. A hit says "others
 * hit this exact crash"; a miss — or an offline/unreachable upstream — says
 * nothing, so this returns null on any failure and never blocks the verdict.
 */
export async function loadCrashCorpus({ offline, url = URLS.crashCorpus } = {}) {
  if (offline) return null
  const doc = await fetchJson(url)
  return Array.isArray(doc?.signatures) ? doc : null
}
